    ...
    get context(){
        if(!this._context){
            ...
            if(con){
                ...
                console.log("create 2dcontext error! w:" + this.canvas.width + " h:"+this.canvas.height);
            }
...
export class MaxRectsBinPack {
     ...
     * 
     */
    insert(width: number, height: number, method: number = 0): Size {
        var newNode:Size;
        this.score1 = 0;
        this.score2 = 0;
        switch (method) {
    ...
    private insert2(Rects: Size[], dst: Size[], method: number): void {
        dst.length = 0;
        ...
        while (Rects.length > 0) {
            ...
            var bestNode: Size = {x:0,y:0,w:0,h:0};
    ...
    private scoreRect(width: number, height: number, method: number,
        score1: number, score2: number): Size {
        var newNode: Size = {x:0,y:0,w:0,h:0};
        score1 = Infinity;
        score2 = Infinity;
        switch (method) {
            case MaxRectsBinPack.BESTSHORTSIDEFIT:
    ...
    private findPositionForNewNodeBottomLeft(width: number, height: number,
        bestY: number, bestX: number) {
        var bestNode = {x:0,y:0,w:0,h:0}
    ...
    private findPositionForNewNodeBestShortSideFit(width: number, height: number) {
        var bestNode = {x:0,y:0,w:0,h:0};
        this.
            //memset(&bestNode, 0, sizeof(Rect));
    ...
    private findPositionForNewNodeBestLongSideFit(width: number, height: number, bestShortSideFit: number, bestLongSideFit: number) {
        var bestNode = {x:0,y:0,w:0,h:0} as Size;
        //memset(&bestNode, 0, sizeof(Rect));
    ...
    private splitFreeNode(freeNode: Size, usedNode: Size): Boolean {
        ...
        let{x,y,w,h} = freeNode;
        let{x:ux,y:uy,w:uw,h:uh} = usedNode
