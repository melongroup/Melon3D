...
export class Texture extends Buffer3D {
    ...
    awaken(): boolean {
        ...
        let { mag, min, mipmap, wrapS, wrapT } = textureData;


        gl.texParameteri(WebGLConst.TEXTURE_2D, WebGLConst.TEXTURE_MAG_FILTER, mag);
        gl.texParameteri(WebGLConst.TEXTURE_2D, WebGLConst.TEXTURE_MIN_FILTER, min);
        // let pepeat = textureData.repeat;
        gl.texParameteri(WebGLConst.TEXTURE_2D, WebGLConst.TEXTURE_WRAP_S, wrapS);   //U方向上设置
        gl.texParameteri(WebGLConst.TEXTURE_2D, WebGLConst.TEXTURE_WRAP_T, wrapT);
    ...
    load(prefix: string, url?: string) {
        ...
        if (LoadStates.WAIT == this.status) {
            ...
            loadRes(prefix, url, this.loadComplete, this, ResType.image);
        }
    }
...
export class CubeTexture extends Texture {
    ...
    awaken(): boolean {
        ...
        g.texParameteri(WebGLConst.TEXTURE_CUBE_MAP, WebGLConst.TEXTURE_MAG_FILTER, mag);
        g.texParameteri(WebGLConst.TEXTURE_CUBE_MAP, WebGLConst.TEXTURE_MIN_FILTER, min);
        g.texParameteri(WebGLConst.TEXTURE_CUBE_MAP, WebGLConst.TEXTURE_WRAP_S, wrapS);   //U方向上设置
        g.texParameteri(WebGLConst.TEXTURE_CUBE_MAP, WebGLConst.TEXTURE_WRAP_T, wrapT);
    ...
    load(prefix: string, url?: string, type: string = '.jpg') {
        if (undefined == url) {
            url = this.data.url as string;
        }
        ...
        if (LoadStates.WAIT == this.status) {
            ...
            for (let i = 0; i < files.length; i++) {
                const face = files[i];
                loadRes(prefix, url + face + type, this.loadComplete, this, ResType.image);
            }
        }
    }
