...
export class IndexBuffer3D extends Buffer3D {
    ...
    awaken(): boolean {
        if (true == this.readly) {
            ...
            if (undefined == this.buffer) {
                console.log("indexBuffer readly is true but buffer is null");
                return false;
            }
            // }
            return true;
        ...
        if (!this.data) {
            this.readly = false;
            console.log("indexData unavailable");
            return false;
        }
        if (undefined == this.buffer) {
    ...
    uploadFromVector(data: number[] | Uint16Array, startOffset: number = 0, count: number = -1): void {
        ...
        if (count != -1) {
            if (data.length - startOffset < count) {
                console.log("VectorData out of range");
                return;
            }
        }
