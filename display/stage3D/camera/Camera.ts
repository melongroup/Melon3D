...
import { PI2, DEGREES_TO_RADIANS } from "../../../core/CONFIG.js";
import { addResize } from "../../../Engine.js";

export type CameraResizeHandler = (width: number, height: number, len: IMatrix3D, far: number, originFar: number, camera?: Camera) => void;
...
export class Camera extends Transform {
    ...
    w: number;
    h: number;
    constructor(cameraResizeHandler: CameraResizeHandler, far: number = 10000, contextMatrix?: IMatrix3D) {
        super();
        this.far = far;
        this.originFar = far;
        this.len = newMatrix3D();
        this.worldTranform = newMatrix3D();
        this.cameraResizeHandler = cameraResizeHandler;
        if (!contextMatrix) {
    ...
    updateWorldMatrix(updateStatus?: number) {
        if (this.status | DChange.trasnform) {
            ...
            this.worldTranform.m3_append(this.len, false, this.worldMatrix);
        }
        this.status = 0;
        return 0;
    }
    ...
    resize(width: number, height: number) {
        let { cameraResizeHandler, len, originFar, far } = this;
        cameraResizeHandler(width, height, len, far, originFar, this);
    }
}
...
export function PerspectiveResize(width: number, height: number, len: IMatrix3D, far: number, originFar: number, camera?: Camera): void {
    let radians = 60 * DEGREES_TO_RADIANS;
    let zNear = 0.5; //500mm
    let zFar = far;
